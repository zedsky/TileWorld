...
public class TWAgentWorkingMemory {
	...
	public DefaultTWPlanner planner; //extension
	private Schedule schedule;
	private TWAgent me;
	private final static int MAX_TIME = 10;//change from 10 to 25
	private final static float MEM_DECAY = 0.5f;

	private ObjectGrid2D memoryGrid;
	
	//extension FuelStation
	public TWEntity fuelStation = null;
	...
	public TWAgentWorkingMemory(TWAgent moi, Schedule schedule, int x, int y) {
		...
		planner = new DefaultTWPlanner();//extension
	}
	...
	public void updateMemory(Bag sensedObjects, IntBag objectXCoords, IntBag objectYCoords, Bag sensedAgents, IntBag agentXCoords, IntBag agentYCoords) {
    	...
    	for (int i = x-r; i <= x+r;i++) {
    		for (int j = y-r; j <= y+r; j++) {
    			if ((i > me.getEnvironment().getxDimension()-1)||(i < 0)||(j > me.getEnvironment().getyDimension()-1)||(j < 0)) {
    				continue;
    			}
		...
		for (int i = 0; i < sensedObjects.size(); i++) {
			...
			//extension update fuel station------------------
			if (o instanceof TWFuelStation) {
				fuelStation = o;
				memoryGrid.set(o.getX(), o.getY(), (TWFuelStation) o);
			}
			//extension--------------------------------------
	...
	//--------------Extension----------------
	public void startingMes() {
		
        //Need tiles case:
        if (me.getTilesNum()<3 && (me.getMemory().getNearbyTile(me.getX(), me.getY(), planner.threshold) == null)) {
        	Map<String, Object> message = new HashMap<String, Object>();
        	Message m = new Message(me.getName(), "all", message);
        	message.put("Need tiles", 1);
        	message.put("Position", new Int2D(me.getX(),me.getY()));
        	me.getEnvironment().receiveMessage(m);
        	System.out.println(me.getName() + " Need tiles");
        }
        //Need holes case:
        if (me.getTilesNum()==3 && me.getMemory().getNearbyHole(me.getX(), me.getY(), planner.threshold) == null) {
        	Map<String, Object> message = new HashMap<String, Object>();
        	Message m = new Message(me.getName(), "all", message);
        	message.put("Need holes", 1);
        	message.put("Position", new Int2D(me.getX(),me.getY()));
        	me.getEnvironment().receiveMessage(m);
        	System.out.println(me.getName() + " Need holes");
        }
	}
	
	/*
	//update provided Bag object
	//Not used
	public void updateProvidedBagOfObjects(Bag Objects) {
		//reset the closest objects for new iteration of the loop (this is short
		//term observation memory if you like) It only lasts one timestep
		closestInSensorRange = new HashMap<Class<?>, TWEntity>(4);
		...
		for (int i = 0; i < Objects.size(); i++) {
			TWEntity o = (TWEntity) Objects.get(i);
	...
	//pack the tiles or holes into a bag
	//Not used
	/*
	public Bag getObjectsInMem(int sx, int sy, double threshold, Class<?> type, Int2D currentGoal) {

		//If we cannot find an object which we have seen recently, then we want
		//the one with maxTimestamp
		Bag ObjectsInMem = new Bag();
			...
			if (me.getEnvironment().isInBounds(x, y) && objects[x][y] != null) {
				...
				if (type.isInstance(o)) {//if it's not the type we're looking for do nothing
					...
					if (this.getSimulationTime() - time <= threshold) {
						//when the object is equal to the current goal, ignore it
						if (o.getX() == currentGoal.getX() && o.getY() == currentGoal.getY()) {
							continue;
						}
	...
	// Personal Extend
	// Detect FuelStation (Not used)
	public TWFuelStation senseFuelStation() {
		return (TWFuelStation) this.getClosestObjectInSensorRange(TWFuelStation.class);
	}
	//
	// Detect Tile
	public TWTile senseTile() {
		return (TWTile) this.getClosestObjectInSensorRange(TWTile.class);
	}
	//Detect Hole
	public TWHole senseHole() {
		return (TWHole) this.getClosestObjectInSensorRange(TWHole.class);
	}
