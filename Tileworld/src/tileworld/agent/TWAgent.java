...
public abstract class TWAgent extends TWEntity implements Steppable {
    ...
    //extension
    public double getTilesNum() {
    	return carriedTiles.size();
    }
    //extension
    
    /*
    //Detect the FuelStation in the range
    //Not use
    //Now use the alternation in workingmemory
    public TWEntity senseFuelStation() {
    	int r = Parameters.defaultSensorRange;
    	ObjectGrid2D ob2D = this.getEnvironment().getObjectGrid();
    	int x = this.getX();
    	int y = this.getY();
    	for (int i = x-r; i <= x+r;i++) {
    		for (int j = y-r; j <= y+r; j++) {
    			if ((i > this.getEnvironment().getxDimension()-1)||(i < 0)||(j > this.getEnvironment().getyDimension()-1)||(j < 0)) {
    				continue;
    			}
    			TWEntity entity = (TWEntity)ob2D.get(i, j);
